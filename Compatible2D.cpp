...
bool Compatible2D::getClickable() {
...
void Compatible2D::setHovered() {

}

void Compatible2D::setClickable(sol::function f) {
	if (!getNode())
		return;
	if (f && !button) {
		clickable = true;
...
void Compatible2D::setHover(sol::function hov) {
	if (!getNode()) return;
...
void Compatible2D::updateButton() {
	if (!clickable)
		return;
